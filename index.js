...
// кладём её в хранилище (по умолчанию, в памяти), связываем с данными пользователя.
//
// Затем подписываем сессию секретом и кладём в cookie `connect.sid`.
//
// При обновлении страницы, мы читаем cookie `connect.sid`,
// получаем из неё id  и смотрим в хранилище нет ли существующей сессии.
//
...
// Определяем функции для сохранения данных пользователя в сессию
...
// Определяем функции для получения данных пользователя из сессию
passport.deserializeUser((profile, done) => {
    // Мы сохранили целиком поэтом данные уже готовы
    done(null, profile);

    // Если бы мы сохранили только id пользователя,
    // то понадобилось бы в начале сходить в неё:
    //
